...
class ProtProcessMovies(ProtPreprocessMicrographs):
    ...
    def _insertAllSteps(self):
        ...
        #finalSteps = self._insertFinalSteps(movieSteps) If running in streaming it is not necessary possible to delete this
    ...
    def _checkNewInput(self):
        ...
        if self.isContinued() and not self.insertedIds:  # For "Continue" action and the first round
            ...
            self.info("Skipping Movies with ID: %s, seems to be done" % skipIds)
    ...
    def _insertNewMoviesSteps(self, newIds):
        ...
        Params:
            newIds: input movies IDs set to be check
        """
        deps = []
    ...
    def processMovieStep(self, movie):
        ...
        if self._filterMovie(movie):
            ...
            convertExt = self._getConvertExtension(movieName)
            correctGain = self._doCorrectGain()

            if convertExt or correctGain:
                inputMovieFn = os.path.join(movieFolder, movieName)
                if inputMovieFn.endswith('.em'):
                    inputMovieFn += ":ems"

                if convertExt:
                    newMovieName = pwutils.replaceExt(movieName, convertExt)
                else:
                    newMovieName = '%s_corrected.%s' % os.path.splitext(movieName)
