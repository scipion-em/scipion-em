# **************************************************************************
# *
# * Authors:    Jorge Jimenez de la Morena (jjimenez@cnb.csic.es)
# *
...
class TestParticleReExtractionWorkflow(TestWorkflow):
    ...
    def testParticleReExtractionWorkflow(self):
        ...
        print("Extracting applying shifts...")
        ...
        print("Extracting particles...r")
        protExtract2 = self.newProtocol(xmippProtcols.XmippProtExtractParticles,
                                        boxSize=64, downsampleType=emprot.OTHER,
                                        doFlip=True, downFactor=8,
                                        runMode=1, doInvert=True)
        protExtract2.inputCoordinates.set(protExtractCoordsShifts.outputCoordinates)
        protExtract2.ctfRelations.set(protCTF.outputCTF)
        protExtract2.inputMicrographs.set(protImport.outputMicrographs)
        self.launchProtocol(protExtract2)
        # self.assertIsNotNone(protExtract2.outputParticles,
        #                      "There was a problem with the extract particles")
        # self.validateFiles('protExtract', protExtract2)
